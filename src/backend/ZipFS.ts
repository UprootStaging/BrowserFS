...
export default class ZipFS extends file_system.SynchronousFileSystem implements file_system.FileSystem {
   ...
   */
  constructor(private input: NodeBuffer | ZipData, private name: string = '') {
    super();
    if (input instanceof ZipData) {
  ...
  static computeIndexResponsive(data: NodeBuffer, index: FileIndex<CentralDirectory>, cdPtr: number, cdEnd: number, cb: (zipData: ZipData) => void, cdEntries: CentralDirectory[], eocd: EndOfCentralDirectory) {
    if (cdPtr < cdEnd) {
      let count = 0;
      ...
      cb(new ZipData(index, cdEntries, eocd, data));
    }
  }

  static computeIndex(data: NodeBuffer, cb: (zipData: ZipData) => void) {
